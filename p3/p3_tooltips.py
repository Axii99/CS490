...
from bokeh.models import Plot, Range1d, MultiLine, Circle, HoverTool, BoxZoomTool, ResetTool, LegendItem, Legend, \
    NodesAndLinkedEdges, EdgesAndLinkedNodes, ColumnDataSource, StaticLayoutProvider
...
for edges in dataset['links']:
    graph.add_weighted_edges_from([(edges['source'],edges['target'],edges['value'])],source=dataset['nodes'][edges['source']-1]['name'],target=dataset['nodes'][edges['target']-1]['name'])
...
node_hover_tool = HoverTool(tooltips=[("index", "@index"),("Name", "@name"),("State", "@state"),("Degree", "@degree")])
edge_hover_tool = HoverTool(tooltips=[("Source", "@source"),("Target", "@target")])
graph_renderer.selection_policy = NodesAndLinkedEdges()
graph_renderer.inspection_policy = NodesAndLinkedEdges()
p.add_tools(node_hover_tool)
graph_renderer.edge_renderer.hover_glyph = MultiLine(line_color=Spectral9[1], line_width=2)
